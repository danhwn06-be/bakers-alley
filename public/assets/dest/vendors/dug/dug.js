                ...
                if (json == undefined) {
                    try {
                        cachedData = JSON.parse(
                            unescape(localStorage.getItem(key)),
                        );
                    } catch (e) {}
                    if (cachedData) {
                    ...
                    try {
                        localStorage.setItem(
                            key,
                            escape(JSON.stringify({ time: now, data: json })),
                        );
                    } catch (e) {
                        console.log(e);
                    }
        ...
        matches.forEach(function (m) {
            ...
            if (scopeName[0] == "#") {
                ...
                if (dataFrag) {
                    ...
                    endFrag = tpl.indexOf(m.replace("#", "/")) + m.length;
                    tpl =
                        tpl.slice(0, startFrag) +
                        rendered +
                        tpl.slice(endFrag, tpl.length);
                }
