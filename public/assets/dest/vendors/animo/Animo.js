...
function Animo() {
    ...
    for (var i = 0; i < rl.len; i++) {
        ...
        for (var j = 0; j < rl._objInfo[i].length; j++) {
            rl._objInfo[i].els[j] = {};
            rl._objInfo[i].els[j].el = rl._objInfo[i].isObj
                ? rl.opt[i].el[j]
                : rl.opt[i].el;

            if (j == 0) {
                rl._objInfo[i].els[j].startTime =
                    new Date().getTime() + rl._objInfo[i].delay;
            } else {
                rl._objInfo[i].els[j].startTime =
                    rl._objInfo[i].els[j - 1].startTime +
                    (rl._objInfo[i].duration +
                        rl._getGap(
                            rl._objInfo[i].duration,
                            rl._objInfo[i].gap,
                        ));
...
Animo.prototype = {
    _animo: function () {
        ...
        function step() {
            ...
            for (
                var i = 0;
                i < objInfo.length;
                i++ // implement another loop for each individual element to check when to start !!!
            ) {
                for (var j = 0; j < objInfo[i].els.length; j++) {
                    if (currentTime < objInfo[i].els[j].startTime)
                        // waiting for animation start
                        continue;

                    if (objInfo[i].els[j].isEnd)
                        // skip elements animation if it is already done
                        continue;
                    ...
                    if (p == 1) {
                        ...
                        var delta = rl.Easing[objInfo[i].easing](
                            p,
                            passTime,
                            0,
                            1,
                    ...
                    objInfo[i].els[j].el.cssText != void 0
                        ? (objInfo[i].els[j].el.style.cssText = newStyle)
                        : objInfo[i].els[j].el.setAttribute("style", newStyle); // add style to the element
                    ...
                    if (p == 1) {
                        ...
                        if (objInfo[i].isDone == objInfo[i].length) {
                            ...
                            if (objInfo[i].callback) objInfo[i].callback();
                        }
                    }
                }
            }
    ...
    getStyle: function (template, delta) {
        ...
        ) // use this version for speed acceleration
        {
            Object.keys(template).forEach(function (key) {
                var buildStyle = rl._buildStyle(
                    template,
                    key,
                    style,
                    css3style,
    ...
    _buildStyle: function (template, key, style, css3style, delta) {
        ...
        } else // for array from - to values
        {
            ...
            for (var z = 0; z < len; z++) {
                if (last == z)
                    newValue +=
                        (template[key][1][z] - template[key][0][z]) * delta +
                        template[key][0][z] +
                        unit;
                else
                    newValue +=
        ...
        if (rl._inArray(key, rl._css3Transforms)) // css3 property
        {
            ...
            rl._inArray(key, rl._specialProp)
        ) // special properties animation
        {
            switch (key) {
                case "background":
	...
	*/
    ...
    _colorAnimo: function (values, delta) {
        var from =
                typeof values[0] == "string" ? rl._toRgb(values[0]) : values[0],
            to =
                typeof values[1] == "string" ? rl._toRgb(values[1]) : values[1];

        return (
            "rgb(" +
            Math.max(
                Math.min(
                    parseInt(delta * (to[0] - from[0]) + from[0], 10),
                    255,
                ),
                0,
            ) +
            ...
            Math.max(
                Math.min(
                    parseInt(delta * (to[1] - from[1]) + from[1], 10),
                    255,
                ),
                0,
            ) +
            ...
            Math.max(
                Math.min(
                    parseInt(delta * (to[2] - from[2]) + from[2], 10),
                    255,
                ),
                0,
            ) +
    ...
    Easing: {
        ...
        easeInElastic: function (p, t, a, b, d) {
            ...
            m < Math.abs(b)
                ? ((m = b), (p = n / 4))
                : (p = (n / (2 * Math.PI)) * Math.asin(b / m));
            return (
                -(
                    m *
                    Math.pow(2, 10 * (t -= 1)) *
                    Math.sin(((t * d - p) * 2 * Math.PI) / n)
                ) + a
            );
        },
        ...
        easeInOutBack: function (p, t, a, b, d) {
            var n = 1.70158;
            if ((t /= d / 2) < 1)
                return (b / 2) * t * t * (((n *= 1.525) + 1) * t - n) + a;
            return (
                (b / 2) * ((t -= 2) * t * (((n *= 1.525) + 1) * t + n) + 2) + a
            );
        },
        ...
        easeOutBounce: function (p, t, a, b, d) {
            return (t /= d) < 1 / 2.75
                ? b * 7.5625 * t * t + a
                : t < 2 / 2.75
                  ? b * (7.5625 * (t -= 1.5 / 2.75) * t + 0.75) + a
                  : t < 2.5 / 2.75
                    ? b * (7.5625 * (t -= 2.25 / 2.75) * t + 0.9375) + a
                    : b * (7.5625 * (t -= 2.625 / 2.75) * t + 0.984375) + a;
        },

        easeInOutBounce: function (p, t, a, b, d) {
            if (t < d / 2)
                return this.easeInBounce(p, t * 2, 0, b, d) * 0.5 + a;
            return (
                this.easeOutBounce(p, t * 2 - d, 0, b, d) * 0.5 + b * 0.5 + a
            );
        },
    },
};
...
while (--i > -1 && !_raf) {
    ...
    _caf =
        window[_p[i] + "CancelAnimationFrame"] ||
        window[_p[i] + "CancelRequestAnimationFrame"];
}
